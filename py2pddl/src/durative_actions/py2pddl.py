...
class Domain:
    ...
    def _generate_requirements():
        return "\t(:requirements :strips :typing :fluents :disjunctive-preconditions :durative-actions)"
...
def action(*Types):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # Subheader
            action_name = f"(:durative-action {func.__name__.replace('_', '-')}"
            ...
            durat, precond, effect = func(*all_args)
            ...
            #duration
            duration = "\t\t:duration (" + " " + str(durat[0]) + " " + "?duration" + " " + str(durat[1]) +")"
            ...
            New_precond_0 = [] 
            New_precond_1 = []        
            for i in range(1, len(precond),  2):
                if i ==1:
                    New_precond_0.append(precond[i])
                    New_precond_1.append(precond[i-1])
                else:
                    New_precond_0.append(precond[i])
                    New_precond_1.append(precond[i-1])
            
            a = []
            for p in New_precond_0: 
                a.append(p.split(" | ")[1])


            precond = [ str("(" + str(New_precond_1[i]) + " " + str(a[i]) + ")") for i in range(0, len(New_precond_1)) ]
            precond = "\t\t:condition " + join(precond, " ")
            ...
            New_effect_0 = [] 
            New_effect_1 = []        
            for i in range(1, len(effect),  2):
                if i ==1:
                    New_effect_0.append(effect[i])
                    New_effect_1.append(effect[i-1])
                else:
                    New_effect_0.append(effect[i])
                    New_effect_1.append(effect[i-1])
            
            a = []
            for e in New_effect_0: 
                a.append(e.split(" | ")[1])

            effect = [ str("(" + str(New_effect_1[i]) + " " + str(a[i]) + ")") for i in range(0, len(New_effect_1)) ]
            effect = "\t\t:effect " + join(effect, " ")

            # Final
            actn = [action_name, repre, duration, precond, effect, "\t)"]
            actn = join(actn, "\n", False)
            actn = actn.replace('_', '-')
